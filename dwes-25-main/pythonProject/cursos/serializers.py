...
class ValoracionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Valoracion
        fields = ("id", "curso", "estudiante", "puntuacion", "comentario", "fecha")
        read_only_fields = ("id", "fecha")

    def validate_puntuacion(self, value):
        if value < 0 or value > 5:
            raise serializers.ValidationError("La puntuación debe estar entre 0 y 5.")
        return value

    def validate(self, attrs):
        if Valoracion.objects.filter(curso=attrs["curso"], estudiante=attrs["estudiante"]).exists():
            raise serializers.ValidationError("Ya existe una valoración de este estudiante para este curso.")
        return attrs
